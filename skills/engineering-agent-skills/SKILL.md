...
description: Use this skill to review software engineering work before shipping and guide implementation quality. It helps with system design review, backend architecture technology choices, system design pattern recognition including large blob/file sync, media streaming, media-heavy social feeds, external data monitoring, streaming Top-K, real-time dispatch, market-data proxies, local search, durable messaging, job scheduling, sandboxed task execution, recommendation feeds, timeline fan-out, auctions/bidding, offline-first sync, live comments, distributed rate limiting, distributed caching, collaborative editing, custom search indexing, coding guidelines, low-level design, commit messages, architecture decisions, API documentation, missing tests, risk areas, and reviewer questions.
---
