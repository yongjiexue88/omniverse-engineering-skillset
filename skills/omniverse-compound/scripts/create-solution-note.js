...
function slugify(value) {
  return value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .replace(/-{2,}/g, "-") || "solution";
}

function parseArgs(argv) {
  const options = { dir: "docs/solutions", force: false, title: null };
  const args = [...argv];
  ...
  if (!options.title) {
    throw new Error("Usage: create-solution-note.js <title> [--dir docs/solutions] [--force]");
  }
